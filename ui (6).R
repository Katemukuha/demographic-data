    ...
    fluidRow(
      column(3, 
             wellPanel(
               ...
               radioButtons(inputId = 'categorical variables', label = "Categorical", choices = c("education", "workclass", "sex"), selected = "education"),    # add radio buttons for categorical variables
                            checkboxInput(inputId = 'is stacked', label = "Stack bars", value = FALSE)     # add check box input for stacked bar chart option
               )
             ),
